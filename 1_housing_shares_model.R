...
library(mgcv)
library(ggplot2)
rdd<-read.csv("Housing Starts Modeling/RDD.csv")
# rdd$MORTGAGE30US[is.na(rdd$MORTGAGE30US)]<-0
# rdd$GDP<-as.numeric(rdd$GDP)
# rdd$Population<-as.numeric(unlist(rdd$Population))
...
s1<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2,data=rdd)
s2<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + hhs,data=rdd)
s3<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + hhs +MORTGAGE30US,data=rdd)
s4<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA,data=rdd)
s5<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US ,data=rdd)
s6<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + TRA97 ,data=rdd)
s7<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA + TRA97 ,data=rdd)
s8<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + TRA97 + PH,data=rdd)
s9<-lm(SFsharepc ~ delPoppc + Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + TRA97 + PH,data=rdd)
s10<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + TRA97 + PH + FHFA,data=rdd)
s11<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA + TRA97 + PH,data=rdd)
s12<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + PH,data=rdd)
s13<-lm(SFsharepc ~ delPop + Quarter + TRA86 + FIRREA +MORTGAGE30US + PH,data=rdd)
s88<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + TRA97 + PH,data=rddsm)
st<-lm(SFsharepc ~ Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + TRA97 + PH,data=rdd)
tab_model(s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,st,show.ci = FALSE)
s8Rob<-coeftest(s8,vcov = NeweyWest(s8,adjust=FALSE,prewhite = FALSE,lag = 2))
tab_model(s8,s12,show.ci = 0)

# should I use change in GDP rather than GDP? (no)
# s81<-lm(SFsharepc ~ delPop + Quarter + del_RLGDPNSA + TRA86 + FIRREA +MORTGAGE30US + TRA97 + PH,data=rdd)
# s82<-lm(SFsharepc ~ delPop + Quarter + del_RLGDPNSAyoy + TRA86 + FIRREA +MORTGAGE30US + TRA97 + PH,data=rdd)
# tab_model(s8,s81,s82,show.ci = 0)

sfs1<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + PH,data=rdd)
sfs2<-lm(SFsharepc ~ delPop + Quarter + TRA86 + FIRREA +MORTGAGE30US + PH,data=rdd)
sfs3<-lm(SFsharepc ~ delPop + Quarter + RES_RLGDP_NSA + TRA86 + FIRREA +MORTGAGE30US + PH,data=rdd)
sfs4<-lm(SFsharepc ~ delPop + Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + PH + TRA97,data=rdd)
sfs5<-lm(SFsharepc ~ Quarter + RLGDP_NSA2 + TRA86 + FIRREA +MORTGAGE30US + PH,data=rdd)
tab_model(sfs1,sfs2,sfs3,sfs4,sfs5,show.ci = 0)
# insert favored model here for predictions
sfsmod<-predict(s8,rddCFsm)
sfsmodact<-s8[["fitted.values"]]
sfcomp<-matrix(c(sfsmod,sfsmodact,rdd$SFsharepc[50:240]),191,3)
